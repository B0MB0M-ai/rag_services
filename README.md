...
`OPENAI_MAX_OUTPUT_TOKENS` limits diagnosis length (default `800`). The Service Assistant uses the
streaming chat endpoint so answer text appears while generation is still in progress.
