...
def test_mock_generator_never_presents_raw_evidence_as_ai_diagnosis() -> None:
    ...
    assert "ปัญหาที่เป็นไปได้" in result.answer
    assert "สาเหตุที่เป็นไปได้" in result.answer
    assert "วิธีตรวจสอบและแก้ไข" in result.answer
    assert "fault code" in result.answer
    assert "ไม่ใช่ผลวิเคราะห์จาก LLM" in result.answer
